...
object CifarCreateLMDBApp {
  ...
  def main(args: Array[String]) {
    ...
    log("create small LMDBs on master")
    // these will never be used, but they are opened when the net is created on the master
    val LMDBCreator = new CreateLMDB(caffeLib)
    LMDBCreator.makeLMDB(trainMinibatchRDD.takeSample(false, 1).iterator, sparkNetHome + "/caffe/examples/cifar10/cifar10_train_lmdb", height, width)
    LMDBCreator.makeLMDB(testMinibatchRDD.takeSample(false, 1).iterator, sparkNetHome + "/caffe/examples/cifar10/cifar10_test_lmdb", height, width)
